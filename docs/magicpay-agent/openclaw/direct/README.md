# Install MagicPay Agent in OpenClaw

MagicPay Agent lets OpenClaw run end-to-end browser workflows with payment, login, approval, and protected secret handling through the combined magicpay-agent CLI.
...
- Release archive: `magicpay-agent-openclaw-v0.1.0.zip`
...
2. Ask OpenClaw to use MagicPay Agent for a simple browser task.
3. Confirm the agent starts by checking `magicpay-agent --version` and `magicpay-agent status`.
4. If OpenClaw does not use MagicPay Agent, make sure `magicpay-agent/` sits directly inside a supported install location.

## Try it out

Once the skill is installed, ask OpenClaw to use MagicPay Agent for a real website task.
...
- **Skill not recognized by OpenClaw**: Make sure the folder is named `magicpay-agent` and sits directly inside `skills/` or `~/.openclaw/skills/`.
...
- **Missing API key**: Run `magicpay-agent init <your-api-key>` or provide the key to OpenClaw and ask it to run init for you. Get a key at https://magicpay.mercuryo.io.
