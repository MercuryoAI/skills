# Install MagicPay Agent in Claude Code

MagicPay Agent lets Claude Code run end-to-end browser workflows with payment, login, approval, and protected secret handling through the combined magicpay-agent CLI.
...
- Release archive: `magicpay-agent-claude-code-v0.1.0.zip`
...
2. Ask Claude Code to use MagicPay Agent for a simple browser task.
3. Confirm the agent starts by checking `magicpay-agent --version` and `magicpay-agent status`.
4. If Claude Code does not use MagicPay Agent, make sure `magicpay-agent/` sits directly inside a supported install location.

## Try it out

Once the skill is installed, ask Claude Code to use MagicPay Agent for a real website task.
...
- **Skill not recognized by Claude Code**: Make sure the folder is named `magicpay-agent` and sits directly inside `.claude/skills/` or `~/.claude/skills/`.
...
- **Missing API key**: Run `magicpay-agent init <your-api-key>` or provide the key to Claude Code and ask it to run init for you. Get a key at https://magicpay.mercuryo.io.
