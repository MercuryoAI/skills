...
MagicBrowse gives Claude Code an LLM-first browser autonomy fallback for real web pages: launch or attach to a Chrome session, drive it with natural-language goals through `magicbrowse act`, and use deterministic `magicbrowse observe` plus `click`/`type`/`fill`/`select`/`press` primitives when autonomy needs single-element precision. Stops at the boundary of any protected form (login, identity, payment) and hands off to MagicPay.
...
```text
Install the `magicbrowse` skill in Claude Code from https://github.com/MercuryoAI/skills/releases/tag/magicbrowse-v0.1.0 using `magicbrowse-claude-code-v0.1.0.zip`. Place the extracted `magicbrowse/` folder directly inside `.claude/skills/` or `~/.claude/skills/`.
...
1. Download `magicbrowse-claude-code-v0.1.0.zip` from https://github.com/MercuryoAI/skills/releases/tag/magicbrowse-v0.1.0.
...
Use this fallback only if you want to manage the `magicbrowse/` folder yourself instead of asking Claude Code to install it from https://github.com/MercuryoAI/skills/releases/tag/magicbrowse-v0.1.0 using `magicbrowse-claude-code-v0.1.0.zip`.

1. Download `magicbrowse-claude-code-v0.1.0.zip`.
