...
```text
Install the `magicpay` skill in Claude Code from https://github.com/nuanu-ai/skills/releases/tag/magicpay-v0.1.48 using `magicpay-claude-code-v0.1.48.zip`. Place the extracted `magicpay/` folder directly inside `.claude/skills/` or `~/.claude/skills/`.
...
<!-- magicpay-continuation:v1 id=claude-direct-builder-plan-apply-1 action=plan-apply -->
...
1. Download `magicpay-claude-code-v0.1.48.zip` from https://github.com/nuanu-ai/skills/releases/tag/magicpay-v0.1.48.
...
2. Ask Claude Code to run `magicpay status`.
3. Ask Claude Code to use MagicPay on a login, identity, checkout, donation, subscription, or payment page.
4. Once a usable or selected URL exists, confirm the agent starts with `magicpay start-session`, then uses `magicpay launch` or `magicpay attach`.
   <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-1 action=plan-apply -->
   After `magicpay plan-fill`, execute its exact returned `applyCommand`.
   <!-- /magicpay-continuation:v1 -->
   Then end with `magicpay end-session`.
5. If Claude Code does not use MagicPay, make sure `magicpay/` sits directly inside a supported install location.
...
<!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-2 action=plan-apply -->
After `magicpay plan-fill`, execute its exact returned `applyCommand`.
<!-- /magicpay-continuation:v1 -->
...
- With a usable or selected checkout URL that is ordinary non-x402, use MagicPay to run `magicpay start-session` and bind the approved checkout browser with `magicpay launch` or `magicpay attach`.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-3 action=plan-apply -->
  After `magicpay plan-fill`, execute its exact returned `applyCommand`.
  <!-- /magicpay-continuation:v1 -->
  Then continue through the page-control owner.
- Use MagicPay to continue a login page after the product session exists and a browser child is bound.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-4 action=plan-apply -->
  After `magicpay plan-fill`, execute its exact returned `applyCommand`.
  <!-- /magicpay-continuation:v1 -->
...
- Use MagicPay on a donation or subscription checkout page; once its usable URL exists, start the product session and bind the browser child.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-5 action=plan-apply -->
  After `magicpay plan-fill`, execute its exact returned `applyCommand`.
  <!-- /magicpay-continuation:v1 -->
...
- Use MagicPay on a payment authorization page; compare live amount, currency, recipient, recurrence, and country, then follow the installed skill's closed normal-checkout `magicpay authorize-payment` command exactly, keeping country outside the command, never adding `--country`, and keeping missing live facts unknown.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-apply-6 action=plan-apply -->
  After `magicpay plan-fill`, execute its exact returned `applyCommand`.
  <!-- /magicpay-continuation:v1 -->
...
- Use MagicPay on an identity-verification form.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-stop-1 action=plan-unavailable-stop -->
...
Use this fallback only if you want to manage the `magicpay/` folder yourself instead of asking Claude Code to install it from https://github.com/nuanu-ai/skills/releases/tag/magicpay-v0.1.48 using `magicpay-claude-code-v0.1.48.zip`.

1. Download `magicpay-claude-code-v0.1.48.zip`.
...
- **Missing API key**: Run `magicpay init <your-api-key>` or provide the key to Claude Code and ask it to run init for you. Get a key at https://app.magiccard.ai/signup.
...
- **Memory plan unavailable**: Confirm the browser is still on the intended page.
  <!-- magicpay-continuation:v1 id=claude-direct-claude-plan-stop-2 action=plan-unavailable-stop -->
  Stop if `magicpay plan-fill` cannot produce safe Memory matches.
  <!-- /magicpay-continuation:v1 -->
