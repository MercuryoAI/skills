...
MagicPay gives OpenClaw a protected-form workflow for prepared browser pages: attach to the current browser, bind a workflow session, discover the supported login, identity, or payment form, request approval for stored values, and finish with `magicpay fill-secret <fillRef> <requestId>` on the normal path.
...
```text
Install the `magicpay` skill in OpenClaw from https://github.com/MercuryoAI/skills/releases/tag/magicpay-v0.1.8 using `magicpay-openclaw-v0.1.8.zip`. Place the extracted `magicpay/` folder directly inside `skills/` or `~/.openclaw/skills/`.
...
1. Download `magicpay-openclaw-v0.1.8.zip` from https://github.com/MercuryoAI/skills/releases/tag/magicpay-v0.1.8.
...
Use this fallback only if you want to manage the `magicpay/` folder yourself instead of asking OpenClaw to install it from https://github.com/MercuryoAI/skills/releases/tag/magicpay-v0.1.8 using `magicpay-openclaw-v0.1.8.zip`.

1. Download `magicpay-openclaw-v0.1.8.zip`.
