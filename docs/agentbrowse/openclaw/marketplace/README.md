...
- If the task needs AI-assisted `observe` or `extract`, ask the agent to request your API key and run `agentbrowse init <your-api-key>`.
- ClawHub slug: `agentbrowse`

## What your agent should do

1. Install the `agentbrowse` skill from ClawHub in the current workspace.
2. Only if the task needs AI-assisted `observe` or any `extract`, request your API key and run `agentbrowse init <your-api-key>`.
...
2. If you use AI-assisted `observe` or `extract` and the local config was reset, rerun `agentbrowse init <your-api-key>`.
